...
def signup(request):
    if request.method == "GET":
	    ...
	    obj.save()
	    send_mail('Verificationmail','Please use this link to activate your account:http://localhost:8000/user/activate/%s'% activation_key,'kushagra343@gmail.com',[request.POST.get('username','')],fail_silently=False)		
...
def forget_password(request):
 if request.method=='POST': 
  ...
  if User.objects.filter(username=user_name).count():
    ...
    obj.save()			
    send_mail('Password Reset Link','Please use this link to reset your password:http://localhost:8000/password_forget/%s'% obj.model_activation_key,'kushagra343@gmail.com',[user_name],fail_silently=False)
